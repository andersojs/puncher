...
uv run python -m puncher --form svg --form png --out iss_tle_1 --cstring  "${TLE_STRING_1}"
uv run python -m puncher --form svg --out iss_tle_1_flat +flatten --cstring  "${TLE_STRING_1}"

uv run python -m puncher --form svg --form png --out iss_tle_2 --cstring  "${TLE_STRING_2}"
uv run python -m puncher --form svg --out iss_tle_2_flat +flatten --cstring  "${TLE_STRING_2}"

uv run python -m puncher --form svg --form png --out charset --testpattern +cellboundaries +punchboundaries +printpunch
uv run python -m puncher --form svg --out charset_flat +flatten --testpattern +cellboundaries +punchboundaries +printpunch